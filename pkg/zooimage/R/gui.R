...
"importImg" <- function ()
{
	...
	if (has(pattern = "^Import_.*[.]zie$")) {
		...
		cat("...OK!\n")
		res <- make.zie(path = dirname(ziefile), Filemap = basename(ziefile),
...
#	if (length(dir) == 0) return(invisible())
	...
	compress.zid.all(path = dir, check.vignettes = check.vignettes,
...
"readTrain" <- function ()
{
	...
	dir <- tkchooseDirectory(initialdir = dir, mustexist = "1",
		title = paste("Select a", getTemp("ZIname"), "training set base dir"))
	dir <- tclvalue(dir)
...
"editDescription" <- function ()
{
	...
	if (res == "1") {	# Create a Zis file
		res <- paste(as.character(tkgetSaveFile(filetypes =
			"{{ZooImage samples description} {.zis}}",
			initialfile = "Description.zis", title = "Create a new ZIS file")),
			collapse = " ")
...
"viewResults" <- function ()
{
	...
	if (any(regexpr("^spectrum of ", Keep) + 1)) {
		...
		Pspec <- select.list(pspec, multiple = FALSE,
			title = "Select taxon for partial spectrum:")
	...
	for (i in 1:length(res)) {
		...
		if (!is.null(spcnames) && length(spcnames) > 0) {
			for (j in 1:length(spcnames)) {
				...
				write.table(spctab,
					file = paste(filenames[i], "_Spectrum_", spcnames[j], ".txt",
						sep = ""),
					quote = FALSE, sep = "\t", eol = "\n", na = "NA",
					dec = getDec(), row.names = FALSE, col.names = TRUE,
...
"optInOutDecimalSep" <- function ()
{
	...
	DecList <- c(".", ",")
	DecSel <- select.list(DecList, preselect = Dec,
		title = "In/Out decimal separator")
...
"RealT" <- function ()
{
	...
	if (res == "Only One Sample") {
		...
		ConvFile <- choose.files(default = ConvFile,
			caption = "Select a conversion file...", multi = FALSE,
			filters = c("Biomass Conversion table (*Conversion.txt)",
			"*Conversion.txt"))
